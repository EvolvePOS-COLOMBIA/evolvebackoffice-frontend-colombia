...
type BranchItemModifierPanelProps = {
  branchId: string
  item: BranchItemResponseDto
}

/**
 * Modificadores de un ítem EN UNA SUCURSAL (contenido usado como pestaña del
 * modal de edición). Interruptor "Usar configuración global" vs configuración
 * propia; en modo global la lista es de solo lectura.
 */
export function BranchItemModifierPanel({ branchId, item }: BranchItemModifierPanelProps) {
  ...
  const modifiersQuery = useItemModifiers(item.itemPublicId, branchId)
  const pickerQuery = useItemsForPicker()
  const deleteMutation = useDeleteItemModifier()

  const useGlobal = item.useGlobalModifiers
    ...
    <div className="space-y-4">
      ...
      {modifiersQuery.isLoading ? (
        ...
        <div className="space-y-4">
          ...
          {useGlobal && (
            <p className="rounded-lg border border-dashed p-3 text-xs text-muted-foreground">{t("readonly_global")}</p>
          )}
        </div>
      )}
